...
export function ActivationPage() {
  ...
  return <><WorkflowSteps current={confirmed ? 5 : 3} /><section className="activation-status card" aria-live="polite"><div className={`activation-icon activation-icon--${kind}`}>{kind === "confirmed" ? "✓" : kind === "rejected" ? "!" : "↻"}</div><h2>{title}</h2><p>{message}</p>{confirmed && <p className="activation-redirect">Opening scheduler status in <strong>{countdown ?? 5}</strong>…</p>}</section></>;
}
