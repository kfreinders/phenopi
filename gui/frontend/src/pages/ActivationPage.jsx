...
export function ActivationPage() {
  ...
  if (!validHash) return <><WorkflowSteps current={analysisEnabled ? 4 : 3} analysisEnabled={analysisEnabled} /><section className="activation-status card"><div className="activation-icon activation-icon--rejected">!</div><h2>Invalid activation link</h2><p>This link does not identify a schedule that can be confirmed.</p><Link className="button-link secondary" to="/schedule">Return to schedule builder</Link></section></>;
  ...
  return <><WorkflowSteps current={confirmed ? (analysisEnabled ? 6 : 5) : (analysisEnabled ? 4 : 3)} analysisEnabled={analysisEnabled} /><section className="activation-status card" aria-live="polite"><div className={`activation-icon activation-icon--${kind}`}>{kind === "confirmed" ? "✓" : kind === "rejected" ? "!" : "↻"}</div><h2>{title}</h2><p>{message}</p>{confirmed && <p className="activation-redirect">Opening scheduler status in <strong>{countdown ?? 5}</strong>…</p>}</section></>;
}
