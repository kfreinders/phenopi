...
export function ScheduleBuilderPage({ edit = false }) {
  const navigate = useNavigate(); const [form, setForm] = useState(null); const [minimum, setMinimum] = useState(""); const [profileSaved, setProfileSaved] = useState(false); const [error, setError] = useState(null); const [saving, setSaving] = useState(false);
  useEffect(() => { api(`/api/schedule/configure?edit=${edit}`).then(data => { if (!edit && data.draft_state === "ready") navigate("/schedule/review", { replace: true }); else { setForm(data.form); setMinimum(data.minimum_start_date); setProfileSaved(data.analysis_profile_saved); } }).catch(setError); }, [edit, navigate]);
  ...
  const submit = async event => { event.preventDefault(); setError(null); if (minimum && form.start_date < minimum) { setError(new Error("Start date cannot be in the past.")); return; } setSaving(true); try { await api("/api/schedule/draft", { method: "POST", body: JSON.stringify(form) }); navigate(form.analysis_enabled ? "/analysis?workflow=schedule" : "/schedule/review"); } catch (reason) { setError(reason); } finally { setSaving(false); } };
  return <><WorkflowSteps current={1} analysisEnabled={form?.analysis_enabled} /><section className="card"><div className="card-header"><div><h2>Schedule builder</h2><p>Step 1: configure when Phenopi should capture images.</p></div></div><ErrorNotice error={error} />{form && <form className="schedule-form" onSubmit={submit}>
    <fieldset><legend>Experiment</legend><div className="grid experiment-details"><TextField label="Experiment name" name="experiment_name" value={form.experiment_name} onChange={update} required maxLength={80} /><TextField label="Researcher" optional name="researcher" value={form.researcher ?? ""} onChange={update} maxLength={80} /></div><label><span className="field-label">Notes <span className="optional">Optional</span></span><textarea name="notes" maxLength="1000" rows="3" value={form.notes ?? ""} onChange={update} /></label>
      <div className="analysis-choice"><div><strong>Canopy measurements</strong><p>Should Phenopi analyze the captured images automatically?</p></div><div className="analysis-choice-options">
        <button type="button" className={!form.analysis_enabled ? "is-selected" : ""} aria-pressed={!form.analysis_enabled} onClick={() => setForm(current => ({ ...current, analysis_enabled: false }))}><span>Images only</span><small>Capture and store photographs without running the canopy pipeline.</small></button>
        <button type="button" className={form.analysis_enabled ? "is-selected" : ""} aria-pressed={form.analysis_enabled} onClick={() => setForm(current => ({ ...current, analysis_enabled: true }))}><span>Analyze canopy</span><small>Calibrate the pipeline, then analyze images automatically between captures.</small>{profileSaved && <i>Saved calibration available</i>}</button>
      </div></div>
    ...
    <div className="actions"><button type="submit" disabled={saving}>{saving ? "Saving experiment…" : form.analysis_enabled ? "Continue to calibration" : "Continue to review"}</button></div>
  </form>}</section></>;
}
